...
const PERFUME_ALLOWED_KEYS = new Set([
  ...
  "rating", "tags", "price", "inspiration",
]);

export async function getPerfumes(status?: Perfume["status"]): Promise<Omit<Perfume, "photo" | "pyramid_image">[]> {
  await init();
  const sql = status
    ? `SELECT id, name, brand, photo_thumbnail, pyramid_thumbnail, description, status, notes_top, notes_heart, notes_base, rating, tags, price, inspiration, created_at FROM perfumes WHERE status = $1 ORDER BY created_at DESC`
    : `SELECT id, name, brand, photo_thumbnail, pyramid_thumbnail, description, status, notes_top, notes_heart, notes_base, rating, tags, price, inspiration, created_at FROM perfumes ORDER BY created_at DESC`;
...
export async function createPerfume(
  ...
  const res = await pool.query(
    `INSERT INTO perfumes (name, brand, photo, photo_thumbnail, pyramid_image, pyramid_thumbnail, description, status, notes_top, notes_heart, notes_base, rating, tags, price, inspiration, created_at)
    ...
    [
      ...
      data.rating ?? null, data.tags ?? null, data.price ?? null,
      data.inspiration ?? null, localISOString(),
    ]
  );
